...
{
  /**
   * Check if inventory_manage table exists (warehouse module)
   * @return bool
   */
  private function inventory_table_exists() {
    return $this->db->table_exists(db_prefix() . 'inventory_manage');
  }
  ...
  public function get_product($id = ''){
    if($id != ''){
      // Check if ware_unit_type table exists (warehouse module)
      if ($this->db->table_exists(db_prefix() . 'ware_unit_type')) {
  ...
  public function get_list_product_by_group($id_chanel, $id_group = '0',$key = '',$limit = 0, $ofset = 1){
    // Only apply inventory filter if warehouse module (inventory_manage table) exists
    $where = '';
    if ($this->inventory_table_exists()) {
    ...
    if($key != ''){
      if($id_group!='0'){
        ...
        $select_list_product = 'select id, description, long_description, rate, tax, group_id from '.db_prefix().'items where id in (select product_id from '.db_prefix().'sales_channel_detailt where sales_channel_id = '.$id_chanel.') and group_id = '.$id_group.' and  description like \'%'.$key.'%\' or rate like \'%'.$key.'%\' or long_description like \'%'.$key.'%\' '.$where.' limit '.$limit.','.$ofset;
        $result = [
        ...
        $select_list_product = 'select id, description, long_description, rate, tax, group_id from '.db_prefix().'items where id in (select product_id from '.db_prefix().'sales_channel_detailt where sales_channel_id = '.$id_chanel.')  and  description like \'%'.$key.'%\' or rate like \'%'.$key.'%\' or long_description like \'%'.$key.'%\' '.$where.' limit '.$limit.','.$ofset;
        $result = [
          'list_product' => $this->db->query($select_list_product)->result_array(),
          'count' => (int)$this->db->query($count_product)->row()->count
        ];
      ...
      if($id_group!='0'){
        ...
        $select_list_product = 'select id, description, long_description, rate, tax, group_id from '.db_prefix().'items where id in (select product_id from '.db_prefix().'sales_channel_detailt where sales_channel_id = '.$id_chanel.') and group_id = '.$id_group.' '.$where.' limit '.$limit.','.$ofset;
        $result = [
          'list_product' => $this->db->query($select_list_product)->result_array(),
          'count' => (int)$this->db->query($count_product)->row()->count
        ];
        ...
        $select_list_product = 'select id, description, long_description, rate, tax, group_id from '.db_prefix().'items where id in (select product_id from '.db_prefix().'sales_channel_detailt where sales_channel_id = '.$id_chanel.') '.$where.' limit '.$limit.','.$ofset;
        $result = [
          'list_product' => $this->db->query($select_list_product)->result_array(),
          'count' => (int)$this->db->query($count_product)->row()->count
        ];
 ...
 */
  ...
  public function get_total_inventory_commodity($commodity_id = false) {
    if ($commodity_id != false) {
      // Check if inventory_manage table exists (warehouse module)
      if (!$this->inventory_table_exists()) {
        // Return object with null inventory if warehouse module not installed
        return (object) ['inventory_number' => null];
      }
