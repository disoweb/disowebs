...
function load_statuses_from_db($statuses)
{
    ...
    $segment2 = $CI->uri->segments[2] ?? '';
    $segment3 = $CI->uri->segments[3] ?? '';
    
    if ($CI->uri->uri_string == 'admin/tasks' || ($segment2 == 'projects' && $segment3 == 'view' && $CI->input->get('group') == 'project_tasks')) {
...
function load_project_statuses_from_db($statuses)
{
    ...
    $segment3 = $CI->uri->segments[3] ?? '';
    $segment4 = $CI->uri->segments[4] ?? '';
    $project_status_once = $CI->project_status_once ?? false;
    
    if (!$project_status_once && ($segment3 == 'project' || $segment3 == 'view') && is_numeric($segment4)) {
        $CI->project_status_once = true;
        $currentProjectStatus = $CI->current_project_status ?? 0;
        $statusesAvalibleToChange = $CI->db->where('project_status_id', $currentProjectStatus)->get(db_prefix() . 'project_status_can_change')->result_array();
