...
class Resource_workload_model extends App_Model
{
    ...
    public function get_data_workload($data_fill, $only_data = false)
    {
                        ...
                        if ($estimate_hour > 0 || $total_time > 0) {
                            if (isset($date_working[$staffid][$f_date]) && $date_working[$staffid][$f_date] == '1') {
                                if ($this->get_total_standard_workload($standard_workload, $date_working[$staffid], $value['staffid'], $value['startdate'], $value['duedate']) < $note['estimate_hour']) {
                                    ...
                                    $staffWorkload = isset($standard_workload[$value['staffid']][$f_date]) ? $standard_workload[$value['staffid']][$f_date] : 0;
                                    if ($estimate_hour >= $staffWorkload) {
                                        $_estimate_hour = $staffWorkload;
                                        $estimate_hour = $estimate_hour - $staffWorkload;
                                    } else {
        ...
        foreach ($data as $val) {
            $int = 5;
            if ($only_data == false) {
                if (isset($data_tooltip_2[$val['staff_id']]) && is_array($data_tooltip_2[$val['staff_id']])) {
            ...
            foreach ($val as $key => $value) {
                if ($only_data == false) {
                    if (!(strpos($key, '_e') === false) || !(strpos($key, '_s') === false)) {
                        $k                 = explode('_', $key);
                        $dateKey = $k[2] . '-' . $k[1] . '-' . $k[0];
                        if (isset($standard_workload[$val['staff_id']][$dateKey]) && $standard_workload[$val['staff_id']][$dateKey] < $value && $value > 0) {
                            $overload[$key] = 1;
                        }
                        if (isset($data_timesheets[$val['staff_id']][$dateKey])) {
                            $leave[$key] = 1;
                        }
                    }
                }
        ...
        foreach ($data['data'] as $workload) {
            foreach ($workload as $key => $value) {
                if (!(strpos($key, '_e') === false)) {
                    $key               = explode('_', $key);
                    $dateKey = $key[2] . '-' . $key[1] . '-' . $key[0];
                    $staffWorkload = isset($standard_workload[$workload['staff_id']][$dateKey]) ? $standard_workload[$workload['staff_id']][$dateKey] : 0;
                    $overload          = $value - $staffWorkload;
                    if ($overload > 0) {
                        $total_overload += $overload;
                        $total_normal += $staffWorkload;
                    } else {
                        $total_normal += $value;
                    }
                }
                ...
                if(is_array($dept_text)){
                    foreach ($dept_text as $department_name) {
                        if (!(strpos($key, '_e') === false)) {
                            ...
                            if ($overload > 0) {
                                ...
                                $department[$department_name]['estimate_normal'] += $staffWorkload;
                            } else {
                        ...
                        if (!(strpos($key, '_s') === false)) {
                            ...
                            if ($overload > 0) {
                                ...
                                $department[$department_name]['spent_normal'] += $staffWorkload;
                            } else {
                    ...
                    if (!(strpos($key, '_e') === false)) {
                        ...
                        if ($overload > 0) {
                            ...
                            $department[$dept_text]['estimate_normal'] += $staffWorkload;
                        } else {
                    ...
                    if (!(strpos($key, '_s') === false)) {
                        ...
                        if ($overload > 0) {
                            ...
                            $department[$dept_text]['spent_normal'] += $staffWorkload;
                        } else {
        ...
        while (strtotime($f_date) <= strtotime($t_date)) {
            if(isset($date_working[$f_date])){
                if ($date_working[$f_date] == 1) {
                    $total += isset($standard_workload[$staffid][$f_date]) ? $standard_workload[$staffid][$f_date] : 0;
                }
            }
