...
class Accounting_model extends App_Model
{
        ...
        foreach ($data_accounts as $data_key => $data_account) {
            ...
            foreach ($data_account as $key => $value) {
                ...
                foreach ($accounts as $val) {
                    ...
                    $formatted_balance = $this->format_money_short($balance, $currency->symbol);
                    $html .= '<li class="list-group-item"><span class="acc-name">'.$name.'</span><span class="badge">'.$formatted_balance.'</span></li>';

                    $data_return[] = ['name' => $name, 'balance' => $balance];
                }
            }
        }
        $html .= '</ul>';
    ...
    /**
     * Format money in short format (1.2K, 3.5M, etc.)
     * @param float $amount
     * @param string $symbol
     * @return string
     */
    public function format_money_short($amount, $symbol = '') {
        $negative = $amount < 0;
        $amount = abs($amount);
