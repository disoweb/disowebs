    ...
    public function accounts_table()
    {
        if ($this->input->is_ajax_request()) {
            ...
            foreach ($rResult as $aRow) {
                ...
                $options = '<a href="' . admin_url('accounting/rp_account_history?account='.$aRow['id']) . '" class="btn btn-default btn-sm" style="border: 1px solid #ccc;" title="' . _l('account_history') . '"><i class="fa fa-history"></i> ' . _l('account_history') . '</a>';
                $row[] =  $options;
