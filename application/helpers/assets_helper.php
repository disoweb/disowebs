...
function _init_admin_assets()
{
    ...
    $adminJsPath = $CI->app_scripts->core_file('assets/js', 'main.js');
    $adminJsVersion = $CI->app_css->core_version();
    $adminJsFullPath = FCPATH . ltrim($adminJsPath, '/');
    if (is_file($adminJsFullPath)) {
        $adminJsVersion .= '-' . filemtime($adminJsFullPath);
    }
    $CI->app_scripts->add(
        'app-js',
        base_url($adminJsPath) . '?v=' . $adminJsVersion,
        'admin',
    ...
    // Add filemtime to ensure CSS updates invalidate cached assets.
    $adminCssPath = $CI->app_css->core_file('assets/css', 'style.css');
    $adminCssVersion = $CI->app_css->core_version();
    $adminCssFullPath = FCPATH . ltrim($adminCssPath, '/');
    if (is_file($adminCssFullPath)) {
        $adminCssVersion .= '-' . filemtime($adminCssFullPath);
    }
    $CI->app_css->add('app-css', base_url($adminCssPath) . '?v=' . $adminCssVersion, 'admin', ['tailwind-css']);
