...
<?php $showSetupMenu = $this->app->show_setup_menu() == true && (is_staff_member() || is_admin()); ?>

<!-- Simple Mobile Dropdown Navigation -->
<div class="mobile-dropdown" id="mobileDropdown">
    <?php if ($showSetupMenu) { ?>
    <div class="mobile-dropdown-tabs">
        <button type="button" class="mobile-dropdown-tab active" data-tab="menu">Menu</button>
        <button type="button" class="mobile-dropdown-tab" data-tab="setup"><?= _l('setting_bar_heading'); ?></button>
    </div>
    <?php } ?>
    
    <div class="mobile-dropdown-scroll">
        <div class="mobile-dropdown-panel active" data-panel="menu">
            <ul class="mobile-dropdown-menu">
                <?php foreach ($sidebar_menu as $key => $item) {
                    ...
                    $submenuId = 'mobile-main-' . $item['slug'];
                ?>
                <li class="mobile-dropdown-item<?= $hasChildren ? ' has-children' : ''; ?>">
                    <?php if ($hasChildren) { ?>
                    <button type="button" class="mobile-dropdown-link" data-submenu="<?= e($submenuId); ?>">
                        <?php if (! empty($item['icon'])) { ?>
                        <i class="<?= e($item['icon']); ?> mobile-dropdown-icon"></i>
                        <?php } ?>
                        <span><?= e(_l($item['name'], '', false)); ?></span>
                        <?php if (isset($item['badge'], $item['badge']['value']) && ! empty($item['badge'])) { ?>
                        <span class="mobile-dropdown-badge"><?= e($item['badge']['value']); ?></span>
                        <?php } ?>
                        <i class="fa fa-chevron-down mobile-dropdown-arrow"></i>
                    </button>
                    <ul class="mobile-dropdown-submenu" id="<?= e($submenuId); ?>">
                        <?php foreach ($item['children'] as $submenu) { ?>
                        <li>
                            <a href="<?= e($submenu['href']); ?>">
                                <?php if (! empty($submenu['icon'])) { ?>
                                <i class="<?= e($submenu['icon']); ?>"></i>
                                <?php } ?>
                    ...
                    <?php } else { ?>
                    <a href="<?= e($item['href']); ?>" class="mobile-dropdown-link">
                        <?php if (! empty($item['icon'])) { ?>
                        <i class="<?= e($item['icon']); ?> mobile-dropdown-icon"></i>
                        <?php } ?>
                        <span><?= e(_l($item['name'], '', false)); ?></span>
        ...
        <?php if ($showSetupMenu) { ?>
        <div class="mobile-dropdown-panel" data-panel="setup">
            <ul class="mobile-dropdown-menu">
                <?php foreach ($setup_menu as $key => $item) {
                    ...
                    $hasChildren = count($item['children']) > 0;
                    $submenuId = 'mobile-setup-' . $item['slug'];
                ?>
                <li class="mobile-dropdown-item<?= $hasChildren ? ' has-children' : ''; ?>">
                    <?php if ($hasChildren) { ?>
                    <button type="button" class="mobile-dropdown-link" data-submenu="<?= e($submenuId); ?>">
                        ...
                        <i class="<?= e($item['icon']); ?> mobile-dropdown-icon"></i>
                        <?php } ?>
                        <span><?= html_purify(_l($item['name'], '', false)); ?></span>
                        <i class="fa fa-chevron-down mobile-dropdown-arrow"></i>
                    </button>
                    <ul class="mobile-dropdown-submenu" id="<?= e($submenuId); ?>">
                        <?php foreach ($item['children'] as $submenu) { ?>
    ...
    <div class="mobile-dropdown-footer">
        ...
        <a href="#" onclick="logout(); return false;" class="logout"><?= _l('nav_logout'); ?></a>
    </div>
</div>
...
<div id="header">
    ...
    <button type="button"
        class="hide-menu tw-hidden md:tw-inline-flex tw-bg-transparent tw-border-0 tw-p-1 tw-mt-4 hover:tw-bg-neutral-600/10 tw-text-neutral-600 hover:tw-text-neutral-800 focus:tw-text-neutral-800 focus:tw-outline-none tw-rounded-md tw-mx-4 ltr:md:tw-ml-4 rtl:md:tw-mr-4 ltr:tw-float-left rtl:tw-float-right">
    ...
    <button type="button" class="mobile-dropdown-toggle" id="mobileDropdownToggle" aria-label="Open menu">
    ...
    <nav>
        <div class="tw-flex tw-justify-between">
            <div class="tw-overflow-hidden tw-shrink-0">
                <div id="logo"
                    class="tw-h-[57px] tw-hidden md:tw-flex tw-items-center tw-pl-4 [&_img]:tw-h-9 [&_img]:tw-w-auto">
